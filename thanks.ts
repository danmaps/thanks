...
const optimize = () => {
const optimal = maxMealPrepTasks(tasks);
setOptimizedTasks(optimal);
setIsOptimized(true);
};
...
const isTaskOptimal = (task: Task) => {
return isOptimized && optimizedTasks.some(t => t.id === task.id);
};

// Calculate overlapping task columns (like Google Calendar)
...
Add your meal prep tasks and find the optimal schedule
</p>
</div>
          ...
          <div className="p-4 bg-amber-600 text-white flex justify-between items-center">
            ...
            <div className="flex gap-2">
              {isOptimized ? (
                <button
                  onClick={reset}
                  className="px-3 py-1 bg-white text-amber-600 rounded-lg text-sm font-medium hover:bg-amber-50 transition-colors"
                >
                  Reset
                </button>
              ) : (
                  ...
                  >
                    <div className="flex justify-between items-start h-full">
                      <div className="flex-1 min-w-0">
                        ...
                        {isOptimal && (
                          <div className="mt-1 inline-block px-2 py-0.5 bg-green-500 text-white text-xs rounded-full font-medium">
                            ✓ Selected
                          </div>
                        )}
                      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          ...
          <p className="text-sm text-gray-700 leading-relaxed">
            The greedy algorithm sorts tasks by end time and picks the earliest-ending
            task that doesn't conflict. This guarantees the maximum number of
            non-overlapping tasks every time.
          </p>
        </div>
      </div>
    </div>
